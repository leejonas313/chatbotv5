...
def find_best_match(user_message, dataset):
    # Check for keywords in the dataset and return corresponding response
    for index, row in dataset.iterrows():
        keywords = row['Keywords'].split(',')  # Assuming the dataset has a 'Keywords' column
        if any(word in user_message for word in keywords):
            return row['Response']  # Assuming there's a 'Response' column with bot responses
    return None

# Define message probability (you can keep this unchanged if needed)
...
# Check all messages and find the best response
